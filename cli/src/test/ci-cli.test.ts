...
let dummyId: string;
const description = "Localnet CLI Test Proposal";
const amount = "1";
...
describe("CLI Integration Tests (using Localnet)", function () {
  this.timeout(15000);
  ...
  it("should switch to Localnet", () => {
    const result = spawnSync("node", [cliPath, "set-network", "localnet"], {
      encoding: "utf-8",
    });
    const stdout = result.stdout || "";
    expect(agent.network).to.equal("localnet");
    results.push(stdout);
  });

  it("should create a proposal on Localnet", async () => {
  ...
  it("should contribute to a proposal on Localnet", async () => {
  ...
  it("should execute a proposal on Localnet", async () => {
